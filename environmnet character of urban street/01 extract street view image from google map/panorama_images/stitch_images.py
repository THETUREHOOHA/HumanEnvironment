...
# read the file
df = pd.read_csv('observation_spots_location.csv', encoding='unicode_escape')
# other parameter
flat_dir = 'tile_download' 
final_directory = 'panorama_stitch' 

# get the cordination from file 

for index, row in df.iterrows():
    Lat = row['Lat']
    Lng = row['Lng']
    FID = row['Fid']
    panoid = panoids(Lat, Lng)
    array = np.array(panoid)


    
# get the latest panoid from each point 
 
    real_panoid = np.flip(array)
    try:
        final_panoid = real_panoid[0]['panoid']
    except Exception as e:
        print(f"Error  final_panoid {final_panoid}: {e}")
        continue
    
    print(final_panoid)
# aquaire the panorama image tiles and stitching them
    tiles = tiles_info(final_panoid)
    download_tiles(tiles, flat_dir, disp=False)
    try:
        stich_tiles(final_panoid, tiles, flat_dir, final_directory)
    except Exception as e:
        print(f"Error stitching tiles for panoid {final_panoid}: {e}")
        continue
